...
public class SmartFormsSoapClientTest {
	
	private static final String WS_ADDRESS = "http://localhost:8088/cds-wsclient/cds-service/";
	...
	{	
		try {
			// Populate the template XML string with the create template identifier, the smart forms document id, a unique patient identifier (for testing), and a valid assigning facility.
			String createRequest = String.format(SMART_FORMS_CREATE_REQUEST1, SMART_FORMS_CREATE_TEMPLATE_ID, SMART_FORMS_DOC_ID, SMART_FORMS_PID_IDENTITY, SMART_FORMS_PID_ASSIGNING_FACILITY);
			System.out.println("Request: " + createRequest);
			// Create the CDS service proxy object that abstracts the remoting mechanism. 
			ClinicalDataServiceSynchronousInterfacePortTypeProxy proxy = new ClinicalDataServiceSynchronousInterfacePortTypeProxy(WS_ADDRESS);
			...
			String response = proxy.createClinicalData(createRequest, SMART_FORMS_CREATE_TEMPLATE_ID, UUID.randomUUID().toString());
